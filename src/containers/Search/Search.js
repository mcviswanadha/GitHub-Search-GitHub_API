...
class RepoSearch extends Component {
  ...
  fetchMoreRepo = () => {
    ...
    let queryString = `is:public ${this.state.repoName} in:name`;
    return this.props.repoSearch(
      GET_ORGANIZATION(queryString, endCursor),
      this.props.token
    );
  };
  repositoryMapper = () => {
    ...
    let moreList = this.props.pageInfo.hasNextPage ? (
      <Button onClick={this.fetchMoreRepo} className={classes.btnList}>
        {" "}
        More List{" "}
      </Button>
    ) : (
      <Button href="#top" className={classes.btnList}>
        No more results,Go To Top
      </Button>
    );
    return (
      <>
  ...
  render() {
    let formElement = (
      <div className="repoBox">
        ...
        >
          ...
          />
          <div>
            <button color="primary" className="btn btn-search" type="submit">
              {" "}
              Search{" "}
            </button>
          </div>
    ...
    let repoCount = this.props.repoCount ? (
      <p className="searchCount">
        {" "}
        Search Result: Total {this.props.repoCount} Repository{" "}
      </p>
    ) : null;
    return (
...
export default withStyles(useStyles)(
  connect(mapStateToProps, mapDispatchToProps)(RepoSearch)
);
